...
model_artm._internal_model.scores.add(artm.TopTokensScore(name="top_words", num_tokens=10))
model_artm._internal_model.scores.add(coherence_score)
model_artm._internal_model.scores.add(artm.PerplexityScore(name='perplexity_score',
                                      dictionary=bv.dictionary))
model_artm._internal_model.scores.add(artm.SparsityPhiScore(name='sparsity_phi_score'))
model_artm._internal_model.scores.add(artm.SparsityThetaScore(name='sparsity_theta_score'))
...
for topic_name in model_artm._internal_model.topic_names:
    print("".center(25, "*"))
    score_tracker = model_artm._internal_model.score_tracker["top_words"]
    for word, prob in zip(score_tracker.last_tokens[topic_name], score_tracker.last_weights[
        topic_name]):
        print("{}: {:.5f}".format(word, prob))

print("".center(25, "*"))
print("sparsity phi score:", model_artm._internal_model.score_tracker[
    'sparsity_phi_score'].last_value)
print("sparsity theta score:", model_artm._internal_model.score_tracker[
    'sparsity_theta_score'].last_value)
print("perplexity: ", model_artm._internal_model.score_tracker['perplexity_score'].last_value)
print("coherence: ", np.mean(model_artm._internal_model
      .score_tracker['TopTokensCoherenceScore'].average_coherence))

plt.plot(model_artm._internal_model.score_tracker['perplexity_score'].value, linewidth=2)
