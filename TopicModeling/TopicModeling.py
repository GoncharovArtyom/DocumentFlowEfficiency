...
ldamodel = models.LdaModel(corpus_tfidf, num_topics=20, passes=5)
ldamodel.save(os.path.join(DATA_DIR,"models/ldamodel20topics"))

#ldamodel = models.LdaModel.load(os.path.join(DATA_DIR,"models/ldamodel"))

for topic in ldamodel.show_topics(formatted=False, num_words=10, num_topics=15):
    for word in topic[1]:
